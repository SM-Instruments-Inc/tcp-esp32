//This code is a TCP/IP high-speed sensing program targeting the ESP32 board. 
//It is designed as non-blocking and sends the board's unique MAC address and 
//sensing data to the server in the form of a structure.
...
//Circular queue for data storage
Queue data_q;
...
int IsEmpty(Queue * q) 
{
    if (q -> front == q -> rear) {  //If front and rear are the same, the queue is empty.
        return true;
    }
    else {
        return false;
...
int IsFull(Queue * q) 
{
    if ((q -> rear + 1) % MAX_SIZE == q -> front) { //If front and rear + 1 are the same, the queue is full.
        return true;
    }
...
static void DataSent(void *pv_parameters)
{
    ...
    int res = getaddrinfo(CONFIG_EXAMPLE_TCP_CLIENT_CONNECT_ADDRESS,
    ...
    while (1) {
        ...
        ESP_LOGI(TAG, "Socket created, connecting to %s:%s",
                CONFIG_EXAMPLE_TCP_CLIENT_CONNECT_ADDRESS,
                CONFIG_EXAMPLE_TCP_CLIENT_CONNECT_PORT);
        ...
        while (1) {
            ...
            if (xmutex != NULL) {
                if (IsFull(&data_q)) {
                    ...
                    int len = SocketSend(TAG, sock, &data_q, sizeof(item_t) * (MAX_SIZE-1));
                    if (len < 0) {
                        ESP_LOGE(TAG, "Error occurred during SocketSend");
                        break;
                    }
